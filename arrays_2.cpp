...
void cheapest(item x[], int length){
    ...
    for (int i=0; i<length; i++){
        if (x[i].price < cheapest_hammer) {
            cheapest_hammer = x[i].price;
            sel_hammer = i;
        }
    };
    // printing the brand and price with lowest price
    cout << "Cheapest hammer is: " << x[sel_hammer].brand << " and the price is: " << x[sel_hammer].price << endl;
}
...
main () {
    ...
    // function call
    cheapest(list_of_items, MAX_ITEMS);
}